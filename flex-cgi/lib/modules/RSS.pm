...
my $query1 = "SELECT * FROM module_settings WHERE module_name='RSS_Module'";
...
$query1 = "SELECT * FROM rss_members WHERE uid='$user_data{uid}'";
...
$query1 = "SELECT * FROM rss_info WHERE id='$default_feed'";
$sth = $dbh->prepare($query1);
$sth->execute || die("Couldn't exec sth!");
while(my @row = $sth->fetchrow)  {
$rss_link = $row[2];
...
$sth->finish();
if (!$rss_link || !$rss_data || !$rss_date) {
    require error; error::user_error($err{bad_input});
    }
...
$rss_date = CGI::Util::expire_calc($feed_expire,'');
    ...
    $rss_data =~ s/\\/&#92;/gso;
    $rss_data =~ s/\G<(.*?)>(?s)(.*?)<\/\1>\G/
    my $ret = "<$1>" . HTML_TEXT::html_escape($2, 1) . "<\/$1>";
    $ret ? $ret : $2;
    /exisg;
    #$rss_data =~ s{<(.*?)(\s.*?)>(?s)(.*?)</\1>}{<$1$2>HTML_TEXT::html_escape($3, 1)</$1>}gso;
           ...
           if (length($rss_data) > 160000) {
               ...
               $rss_data = $dbh->quote($rss_data);
     require SQLEdit;
     my $string = qq(UPDATE `rss_info` SET `data`=$rss_data, `date`='$rss_date' WHERE `id` ='$default_feed' LIMIT 1 ;);
...
#$generator[0] = HTML_TEXT::html_escape($generator[0], 1) if $generator[0];
       ...
       require DATE_TIME;
       $rss_date = DATE_TIME::format_date($rss_date, 5);
...
if($user_data{sec_level} eq $usr{admin} || $user_data{uid} eq $feed_mod) {
    ...
    if ($user_data{uid} ne $usr{anonuser}) {
        ...
        require theme;
        my $user_status = theme::box_header('RSS Feed');
        $user_status .= <<HTML;
<tr>
<td valign="top">
$admin_html
<IFRAME width="173" height="250" SRC="$cfg{pageurl}/index.$cfg{ext}?op=feeds;module=RSS;id=$default_feed" marginwidth="1" marginheight="1" border="1" frameborder="1"></IFRAME><br>
$user_html
</td>
</tr>
HTML
...
$select1 = '' if !$show_logo;
      $request_feed = qq(You can Request a RSS Feed Here <a href="$cfg{pageurl}/index.$cfg{ext}?op=view_pm;message=send;to=$feed_mod">Request Feed</a><br>) if $request_feed;
      require theme;
      theme::print_header();
      theme::print_html($user_data{theme}, 'RSS Settings');
print <<HTML;
...
<input type="hidden" name="op" value="save_set">
...
HTML
      theme::print_html($user_data{theme}, 'RSS Settings', 1);
...
if ($rssid) {
     require SQLEdit;
     my $string = qq(UPDATE `rss_members` SET `rss_id`='$select', `options`='$options' WHERE `uid` ='$user_data{uid}' LIMIT 1 ;);
     SQLEdit::SQLAddEditDelete($string);
}
 else {
     require SQLEdit;
     my $string = qq(INSERT INTO rss_members VALUES (NULL,'$user_data{uid}','$select','$options'););
     SQLEdit::SQLAddEditDelete($string);
 }
                # Redirect to user_actions page.
                print $query->redirect(-location => $cfg{pageurl} . '/index.' . $cfg{ext} . '?op=settings;module=RSS');
...
<table width="95%" border="1" cellspacing="0" cellpadding="4">
<form name="form1" method="post" action="">
<input type="hidden" name="op" value="admin_add">
<input type="hidden" name="module" value="RSS">
...
$sth->finish();
 my $seclvl = '';
    foreach (sort keys %usr) {
      ...
      require theme;
      theme::print_header();
      theme::print_html($user_data{theme}, 'RSS Admin Settings');
print <<HTML;
<table border="0" cellpadding="4" cellspacing="0" width="95%" class="navtable">
<tr>
...
<input type="hidden" name="op" value="admin_set">
<input type="hidden" name="module" value="RSS">
...
error::user_error($err{auth_failure});
   ...
   $options = $dbh->quote($options);
     require SQLEdit;
     my $string = qq(UPDATE `module_settings` SET `settings`=$options WHERE `module_name`='RSS_Module' LIMIT 1 ;);
     SQLEdit::SQLAddEditDelete($string);

                # Redirect to user_actions page.
                print $query->redirect(-location => $cfg{pageurl} . '/index.' . $cfg{ext} . '?op=admin;module=RSS');
...
$rss_data = $res->content;
    ...
    $rss_data =~ s/'/&#39;/gso;
    $rss_data =~ s/\\/&#92;/gso;
    $rss_data =~ s{<(.*?)>(?s)(.*?)</\1>}{<$1>HTML_TEXT::html_escape($2, 1)</$1>}gso;
...
if ($id) {
     ...
     $link = $dbh->quote($link);
     $rss_data = $dbh->quote($rss_data);
     $seclvl = $dbh->quote($seclvl);
     my $string = qq(UPDATE `rss_info` SET `name`=$name, `link`=$link, `data`=$rss_data, `date`='$fdate', `sec_lvl`=$seclvl WHERE `id` ='$id' LIMIT 1 ;);
     SQLEdit::SQLAddEditDelete($string);
}
 else {
     require SQLEdit;
     ...
     $rss_data = $dbh->quote($rss_data);
     $seclvl = $dbh->quote($seclvl);
     my $string = qq(INSERT INTO rss_info VALUES (NULL,$name,$link,$rss_data,'$fdate',$seclvl););
     SQLEdit::SQLAddEditDelete($string);
 }
                # Redirect to user_actions page.
                print $query->redirect(-location => $cfg{pageurl} . '/index.' . $cfg{ext} . '?op=admin;module=RSS');
