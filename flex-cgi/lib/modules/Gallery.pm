...
require error;
    mkdir("$cfg{gallerydir}/$options", 0777) or error::user_error("Cant Make Folder $options. ($!)");
        require theme;
        theme::print_header();
        theme::print_html($user_data{theme}, "Gallery");
        admin_menu();
...
SQLEdit::SQLAddEditDelete($sql);
require error;
    mkdir("$cfg{gallerydir}/$cat/$options", 0777) or error::user_error("Cant Make Folder 1. ($!)");
        require theme;
        theme::print_header();
        theme::print_html($user_data{theme}, "Gallery");
        admin_menu();
...
print $query->redirect(-location => $cfg{pageurl} . '/index.' . $cfg{ext} . '?op=sub_cat;module=Gallery;cat=' . $cat);
  ...
  require error;
  error::user_error("There was nothing to do.");
 }
}
